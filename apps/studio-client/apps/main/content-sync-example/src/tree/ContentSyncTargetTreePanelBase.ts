...
class ContentSyncTargetTreePanelBase extends TreePanel {
  ...
  constructor(config: Config<ContentSyncTargetTreePanel> = null) {
    ...
    asConfig(as(this.getStore(), TreeStore)).root = this.#rootObj;
  }
  ...
  #handleRemoveTargetTree(node: FolderTreeNode): void {
    ...
    var nonLeafchildNodes: Array<any> = node.childNodes || [];
  ...
  #handleContentListChange(ev: PropertyChangeEvent): void {
    var oldPropertyValue: Array<any> = ev.oldValue;
    var newPropertyValue: Array<any> = ev.newValue;

    var contentList: Array<any> = ev.newValue;
    var treeStore = as(this.getStore(), TreeStore);
    ...
    if ((newPropertyValue.length - oldPropertyValue.length) >= 0) {
      var insertContents = contentList.filter((item: any): boolean =>
      ...
      var deleteContents = oldPropertyValue.filter((item: FolderTreeNode): boolean =>
  ...
  #handleRemove(item: FolderTreeNode): void {
    var toBeDeleted = as(this.getStore().getById(item.data.id), FolderTreeNode);
    if (toBeDeleted) {
      toBeDeleted.remove();
    } else {
  ...
  #handleAdd(item: FolderTreeNode): void {
    var path = ContentSyncTargetTreePanelBase.#getPathToRoot(item);
    var currentChild: any;
    var parent: any = asConfig(this).store.getRoot();
  ...
  static #createEntryForId(item: FolderTreeNode): any {
    ...
    var folderNode = new FolderTreeNode({
      id: itemId,
      text: item.data.text,
      expanded: true,
      leaf: item.data.leaf,
