...
class ExcludeListRadioGroupBase extends Panel {
  ...
  get modelProperty():string { return this.#modelProperty; }
  set modelProperty(value:string) { this.#modelProperty = value; }
  ...
  constructor(config:Config<ExcludeListRadioGroup> = null) {
    ...
    var setting =as( this.modelBean.get(ContentSyncConstants.SELECTED_ENVIRONMENT_SETTING),  ContentSyncSettings);
    if (setting) {
  ...
  #handleSetting(setting:ContentSyncSettings):void {
    ...
    if (items) {
      items.forEach((item:string):void => {
        var checkBox = new Checkbox(Config(Checkbox, {
          boxLabel: item,
          name: item,
          itemId: item.concat("_checkbox" + this.modelProperty),
          listeners: {
            change: {
              fn: bind(this,this.#handleChange)
            }
          }
        }));
        this.add(checkBox);
  ...
  #handleChange(changed:any):void {
    var excludes:Array<any> = this.modelBean.get(this.modelProperty);
    var checkBoxName:string = changed.name;
    if (changed.checked) {
      this.modelBean.set(this.modelProperty, [checkBoxName].concat(excludes));
    } else {
      var clearedList:any = excludes.filter((it:any):boolean => 
         it !== checkBoxName
      );
